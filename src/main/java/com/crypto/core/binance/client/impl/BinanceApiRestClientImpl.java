...
import com.crypto.core.binance.client.domain.general.BinanceResponse;
import com.crypto.core.binance.client.domain.general.ExchangeInfo;
import com.crypto.core.binance.client.domain.general.UserAsset;
...
import static com.crypto.core.binance.client.impl.BinanceApiServiceGenerator.executeSync;
...
public class BinanceApiRestClientImpl implements BinanceApiRestClient {
	...
	public List<UserAsset> getUserAssets(boolean needBtcValuation) {
		...
		BinanceResponse<List<UserAsset>> binanceResponse = executeSync(binanceApiService.getUserAssets(url, needBtcValuation));
	...
	public List<Asset> getAllAssets() {
		...
		BinanceResponse<List<Asset>> binanceResponse = executeSync(binanceApiService.getAllAssets(url));
		if (!binanceResponse.isSuccess()) {
			throw new BinanceApiException(binanceResponse.getMessage());
		}
		return binanceResponse.getData();
	...
	public List<Asset> listAssets(Long recvWindow, Long timestamp) {
		return executeSync(binanceApiService.listAssets(recvWindow, timestamp));
	}
}
